...
async function main() {
  ...
  // ALERT_DAYS=30 notifica TODOS com ate 30 dias restantes
  const maxDays = Math.max(...CONFIG.alertDays);
  const toNotify = deals.filter(d => {
    if (d.daysLeft === null) return false;
    // Ja venceu ha ate 7 dias - ainda avisa
    if (d.daysLeft < 0 && d.daysLeft >= -7) return true;
    // Vence dentro do prazo configurado
    return d.daysLeft >= 0 && d.daysLeft <= maxDays;
  });
