...
async function fetchDeals() {
  console.log('📡 Buscando deals dos funis fixos...');
  // IDs fixos — Licitações RC Scientific=11, Licitações Line=16
  const targets = [
    { id: 11, name: 'Licitações RC Scientific' },
    { id: 16, name: 'Licitações Line' },
  ];
  console.log(`✓ Funis: ${targets.map(p => p.name).join(', ')}`);
