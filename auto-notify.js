...
async function fetchDeals() {
  ...
  if (!pipes.length) {
    console.error('✗ Funis "Licitações RC Scientific" e "Licitações Line" não encontrados. Abortando.');
    process.exit(1);
  }
  console.log(`✓ Funis encontrados: ${pipes.map(p => p.name).join(', ')}`);

  let raw = [];
  const targets = pipes; // SOMENTE os funis de licitação — nunca carrega tudo
